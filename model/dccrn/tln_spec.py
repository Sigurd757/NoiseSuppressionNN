...
class TLN_Fb(nn.Module):
    def __init__(self, ) :
        super(TLN_Fb, self).__init__()
        
        fix=True
        self.fix = fix
        self.stft = ConvSTFT(320, 160, 512, 'hann', 'complex', fix=fix)
        self.istft = ConviSTFT(320, 160, 512, 'hann', 'complex', fix=fix)
        
        self.clp = ComplexLinearProjection(256)
        self.bn = nn.BatchNorm1d(256, affine=False)
        
        self.dfsmn1 = DfsmnLargeDecoder(5, 256, 128, 40, 39)
    ...
    def forward(self, x):
        ...
        x2 = self.dfsmn1(specs)
        x2 = self.conv2(x2)
        # print(x2.shape)
        # print(x2.type())
        
        r_mask, i_mask = torch.chunk(x2,2,1)
